...
namespace u8lib{
  ...
  std::string to_cppstring(u8string s){
    ...
    for (int i = 0; i < s.length; i++){
      std::string tmp = u8lib::translateRange(s.characters[i].decimal, u8lib::checkControl(s.characters[i+1].decimal));
      ss.write(tmp.c_str(), tmp.length());
    }
